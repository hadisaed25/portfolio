...
smallImages.forEach(function(image) {
  image.addEventListener("click", function() {
    // Toggle 'big-image' class to enlarge/reduce the image
    if (this.classList.contains("big-image")) {
      this.classList.remove("big-image"); // If already big, make it small
    } else {
      // Remove the class from other images if any are already enlarged
      smallImages.forEach(function(img) {
        img.classList.remove("big-image");
      });
      this.classList.add("big-image"); // Make the clicked image big
    }
  });
});
